  ...
  traverse(ast, {
    Identifier(path: LooseObject) {
      ...
      if (path.node.name === '$slots') {
        const SlotName =
          _.get(path, 'parentPath.parent.property.name', '') ||
          _.get(path, 'parentPath.parent.property.value', '')
 ...
 * 不是‘@’开头，就是普通注释文本
 */
function parseComments({ commentsText, rootCommentType, propEventKey }: CommentsPaylod_Interface) {
...
function ClassProperty_Visitor(path: LooseObject) {
  ...
  _.forEach(_.get(path, 'node.decorators', []), (decorator: LooseObject) => {
  ...
  const commentsText: string = _.get(path, `node.leadingComments[${coLength - 1}].value`, '') + ''
