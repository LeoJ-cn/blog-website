...
export const getEmitSchema = function (
  tsType: string,
  emitHandlerName: string,
  payloadParams: any[],
  __SchemaWordbook: LooseObject,
) {
  const validMethodName = emitHandlerName.replace(/[^a-zA-z0-9]/gim, '_')
...
export const getPropSchema = function (
  tsType: string,
  defaultValue: string,
  propName: string,
  __SchemaWordbook: LooseObject,
...
function getMetodPayloadList(
  ...
  return payloadParams.map((item: string) => {
    ...
    const [key, varSymbol, keyType] = item
      .replace(/\s/gim, '')
      .replace(/(.+?)([?!]?[:=]+)(.+)/gim, `$1${splitChar}$2${splitChar}$3`)
      .split(splitChar)
    const isRequired = key.indexOf('?') === -1
    const varKey = key.replace(/\?|\!/gim, '')
...
function tsType2JsonSchema(
  tsVariableName: string,
  tsType: string,
  tsExpression = ':',
  __SchemaWordbook: LooseObject,
): JsonType {
