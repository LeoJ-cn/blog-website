...
export const parseSchema = async function (filePath: string, originalJsonSchema: LooseObject, schemaWordbook: LooseObject) {

  let legalTsxText
  const testFilePath = resolve(filePath);
  const newTsxPath = path.join(
  ...
  const SchemaWordbookStr = Object.keys(schemaWordbook).reduce(
    function (pre, tsVariableName) {
      const {
        tsType,
        tsExpression
      } = schemaWordbook[tsVariableName];
      return `${pre}
      ${tsVariableName}${tsExpression} ${tsType};`
    },
    ''
  )
  legalTsxText = legalTsxText.replace(/(export.*?default.*?class.*?Vue.*?{)/i, `$1  \n ${SchemaWordbookStr} \n`)
   ...
   */
  legalTsxText = legalTsxText.replace(/\*\s*\@type/img, '* @type_ss')
  ...
  const definedSchema = getTransformData(
    baseData,
    compTag
  );
  ...
  Object.keys(events).forEach(eventKey => {
    ...
    const {
      __comment__ = `默认注释：@Prop-${eventKey}`
    } = commentsMap.events[eventKey]

    events[eventKey] = _.merge(
      tsAstViewerEventJsonSchema,
      {
        label: __comment__,
...
function getTransformData(definedSchema: LooseObject, tag: string): LooseObject {
  ...
  tagProsSchema = removeRefFromSchema(
    tagProsSchema,
    '移除 schema.$ref 指针:1-1'
  )
...
function getSchemaWithReplacedRef(oldSchema: LooseObject, defineMap: LooseObject, processTitle: string) {
  ...
  needReplaceKeyVal.forEach((item: NeedReplaceObj) => {
     ...
     */
    const refType = String(ref).replace(/.*\/(.+)/img, '$1')
...
function removeRefFromSchema(oldSchema: LooseObject, processTitle: string) {
  ...
  Object.keys(
    _.get(newSchema, 'properties', {})
  ).forEach((topKey: string) => {
    /**
     * 性能优化:
    ...
    if (/^__/.test(topKey)) {
      deepWalk(
        _.get(newSchema, `properties.${topKey}`, {}),
        `properties${keySplitSymbol}${topKey}`
      )
    }
  })
