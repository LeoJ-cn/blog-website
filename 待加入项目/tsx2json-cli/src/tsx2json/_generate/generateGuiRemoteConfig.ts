...
export function getRemoteConfig(guiconfig: GuiConfigParams_Interface): RemoteConfig_Interface {
  const { APP_ID, NPM_NAME, VERSION, PACKAGE_NAME, COMPONENT_NAME, REMOTE_PATH } = guiconfig
  return {
    path: {
      type: 'remote',
      appid: `${APP_ID}`,
...
export function getDependConfig(guiconfig: GuiConfigParams_Interface): DependConfig_Type {
  const { APP_ID, VERSION, NPM_NAME, PACKAGE_NAME, COMPONENT_NAME } = guiconfig
  return [
    {
      type: 'async-service',
      value: {
...
export function generateGuiRemoteConfig(filePath: string, componentName: string = '') {
  ...
  for (let i = 0; i < 50; i++) {
    ...
    const isInPackages = path.basename(path.resolve(walk_path, '..', '..')) === 'packages'
  ...
  if (!ALL_CONFIG.COMPONENTS_PATH) {
    throw new Error(
      '【失败：生成远端配置】没有匹配packages的包，请检查组件是否存放在 /packages/{packageName}/components/* 下',
    )
  }

  // 查找components所在的package目录
  let COMPONENTS_INDEX_TS_PATH = path.resolve(ALL_CONFIG.COMPONENTS_PATH, '..', 'index.ts')
  if (!fs.existsSync(COMPONENTS_INDEX_TS_PATH)) {
    COMPONENTS_INDEX_TS_PATH = path.resolve(ALL_CONFIG.COMPONENTS_PATH, '..', 'index.js')
    if (!fs.existsSync(COMPONENTS_INDEX_TS_PATH)) {
  ...
  // 项目根目录
  ALL_CONFIG.ROTE_PATH = path.resolve(ALL_CONFIG.COMPONENTS_PATH, '..', '..', '..')

  // config.json（获取appid）
  ALL_CONFIG.CONFIG_JSON_PATH = path.resolve(ALL_CONFIG.ROTE_PATH, 'packages', 'config.json')

  // package.json (获取name以及version)
  ALL_CONFIG.PACKAGE_JSON_PATH = path.resolve(ALL_CONFIG.ROTE_PATH, 'package.json')

  const { PACKAGE_JSON_PATH, CONFIG_JSON_PATH } = ALL_CONFIG

  const { name, version } = require(PACKAGE_JSON_PATH)
  ALL_CONFIG.NPM_NAME = name
  ALL_CONFIG.VERSION = version

  if (!name || !version) {
    throw new Error(`【失败：生成远端配置】package.json缺少name或version：${PACKAGE_JSON_PATH};`)
  }

  const { appid } = require(CONFIG_JSON_PATH)
  ALL_CONFIG.APP_ID = appid

  if (!appid) {
    throw new Error(`【失败：生成远端配置】config.json缺少appid：${CONFIG_JSON_PATH};`)
  }

  // TODO: 这里通过正则获取 package.name 有风险
  let packageName = ''
  const str = fs.readFileSync(COMPONENTS_INDEX_TS_PATH, 'utf8')
  String(str).replace(/name\s*:\s*['"](.+)['"]/gim, (match, $1) => {
    packageName = $1
    return match
  })
