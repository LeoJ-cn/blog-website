...
function initCompilerApi() {
  return getCompilerApi(OPTION_CONFIG.compilerPackageName)
    .then((cacheApi) => {
      setRootApi(cacheApi)
    })
    .catch((err) => {
...
function initPublicApi() {
  return getPublicApiInfo(OPTION_CONFIG.compilerPackageName)
    .then((publicApiInfo) => {
...
function generateFileDefinition(
