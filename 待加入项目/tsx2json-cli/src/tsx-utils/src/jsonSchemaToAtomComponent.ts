...
export const jsonSchemaToAtomComponent = function (
  JsonData: Cli_ComponentJsonschema_Interface,
): Utils_JsonSchemaToAtomComponentReturn_Type {
  return [getAtomBoxEl(), getAtomContentEl(JsonData), ...getAtomSlotList(JsonData)]
}
...
function getAtomContentEl(
  JsonData: Cli_ComponentJsonschema_Interface,
): Utils_AtomComponentRenderContent_Interface {
  const { tag, props } = JsonData
...
function getAtomSlotList(JsonData: Cli_ComponentJsonschema_Interface): LooseObject[] {
  ...
  lists.forEach((item, index) => {
    ...
    slotList.push(_.merge({}, slotConfig, slotsData))
  })
  return slotList
}
