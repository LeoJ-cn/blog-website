...
export default class Expression {
    ...
    forEach(this.TokenList, (item, index) => {
      ...
      if (item.type === TOKEN_TYPE_OPERATOR) {
        ...
        if (currentExpression.expressionType === EXPRESSION_TYPE_FIELD &&
            ...
            (nextToken && (nextToken.type === TOKEN_TYPE_SPACE || nextToken.type === TOKEN_TYPE_LOGIC || nextToken.type === TOKEN_TYPE_OPERATOR))) {
          this.endExpression();
        }
      }
    });
  ...
  endExpression() {
    ...
    let {
      offset,
      value
    } = _getExpressionOffset(currentExpression);
  ...
  endAllExpression() {
    if (this.ExpressionList.length > 1) {
      ...
      let {
        offset,
        value
      } = _getExpressionOffset(this.ExpressionTree);
