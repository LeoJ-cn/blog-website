 ...
 * 60001    字段表达式格式错误
 */
import { last, forEach, includes, concat, get, startsWith, endsWith, find, toNumber } from 'lodash'
import { isDateValue, isIntegerValue, isCorrectRange } from '../utils/validationUtils.js'
import { FIELDS_LIST, DATE_TYPE_FIELDS, RANGE_TYPE_FIELDS } from '../config/index'
import {
  TOKEN_TYPE_OPERATOR,
...
export default {
  ...
  validateExpression() {
    ...
    let patentField =
      patnetFieldTokenType === TOKEN_TYPE_FIELD
        ? get(this.parentExp, 'children[0].data.value', '').toLocaleUpperCase()
        : ''
    if (this.currentExp.expressionType === EXPRESSION_TYPE_OPERATOR) {
      if (
        last(children).expressionType !== EXPRESSION_TYPE_TOKEN ||
        last(children).data.value !== ')'
      ) {
        //  缺少小括号和中括号作为token 错误
        // errorData.errorCode = '30002';
      } else if (
        children.length === 2 ||
        (children.length === 3 && get(children[1], 'data.type') === TOKEN_TYPE_SPACE)
      ) {
      ...
      if (
        ...
        if (includes(DATE_TYPE_FIELDS, patentField) || includes(RANGE_TYPE_FIELDS, patentField)) {
          if (
            rangeTokenList.length !== 3 ||
            get(rangeTokenList, '[1].data.value').toLocaleUpperCase() !== 'TO'
          ) {
    ...
    forEach(this.currentExp.children, (item, index) => {
      if (item.expressionType === EXPRESSION_TYPE_TOKEN) {
        ...
        if (item.data.type === 'field') {
          ...
          item.data.type === TOKEN_TYPE_PLACESYMBOL &&
          !/^\$(W(\d+|S)$)|(PRE\d+$)/.test(item.data.value.toLocaleUpperCase())
        ) {
          errData.errorCode = '40001'
          tokenError.push(errData)
        } else if (
          item.data.value === '(' &&
          get(last(this.currentExp.children), 'data.value') !== ')'
        ) {
          errData.errorCode = '30002'
          tokenError.push(errData)
        } else if (
          item.data.value === ')' &&
          (index !== this.currentExp.children.length - 1 ||
            get(this.currentExp, 'children[0].data.value') !== '(')
        ) {
          ...
          if (/^"\s*"$|^'\s*'$/.test(item.data.value)) {
            ...
            (startsWith(item.data.value, '"') && !endsWith(item.data.value, '"')) ||
            item.data.value === '"'
          ) {
  ...
  validateRangeToken(rangeTokenList, patentField) {
    ...
    if (includes(DATE_TYPE_FIELDS, patentField)) {
      ...
      if (
        isDateValue(rangeStart) &&
        isDateValue(rangeEnd) &&
        !isCorrectRange(rangeStart, rangeEnd)
      ) {
        let errData = {}
        errData.id = rangeTokenList[0].data.id
      ...
      if (
        isIntegerValue(rangeStart) &&
        isIntegerValue(rangeEnd) &&
        toNumber(rangeStart) >= toNumber(rangeEnd)
      ) {
        let errData = {}
        errData.id = rangeTokenList[0].data.id
