...
export default class Suggestion {
  ...
  getQueryTextWidthSuggeston(dropData) {
    ...
    if (section !== SECTION_TYPE_FIELD && section !== SECTION_TYPE_LOGIC && section !== SECTION_TYPE_HISTORY) {
      text = `\"${text}\"`;
      if (section === SECTION_TYPE_KEYWORDS) {
        text = `OR ${text}`;
      }
      if (section === SECTION_TYPE_ANCS && get(this.cursorWordData, 'cursorFieldExp.expressionType') !== EXPRESSION_TYPE_FIELD) {
        text = `ANCS:(${text})`;
      }
      // 表达式中包含左括号则自动补全右括号
      if (get(this.cursorWordData, 'cursorExp.expressionType') === EXPRESSION_TYPE_OPERATOR) {
        ...
        if (get(children, '[0].data.value') === '(' && get(children, `[${children.length - 1}].data.value`) !== ')') {
          text += ')';
        }
      }
    }
    ...
    if ((section === SECTION_TYPE_AUTOCOMPLETE || section === SECTION_TYPE_ANCS) && stemStartToken.start) {
  ...
  getSuggestionByCursor(range) {
    ...
    let fieldName = get(this.cursorWordData, 'cursorFieldExp.children[0].data.value', '').toLocaleUpperCase();
  ...
  prepareCursorWordData(expressionTree, pos) {
    ...
    if (this.cursorWordData.cursorExp && !this.cursorWordData.cursorFieldExp && expressionTree.expressionType === EXPRESSION_TYPE_FIELD) {
  ...
  getSuggestionFnDataByCursor() {
    ...
    if (cursorFieldExp) {
      ...
      if (includes(RANGE_TYPE_FIELDS, fieldName) && cursorToken.data.value === OPERATOR_RANGE_LEFT) {
        suggestionFnData.typeList.push(REQUEST_TYPE_RANGE);
      }
      if (includes(RANGE_TYPE_FIELDS_SHORT, fieldName) && cursorToken.data.value === OPERATOR_RANGE_LEFT) {
      ...
      if (cursorExp.expressionType === EXPRESSION_TYPE_OPERATOR &&
        (cursorExp.offset.start < this.cursorRange.start && this.cursorRange.end < cursorExp.offset.end) &&
    ...
    if (currentIndex > 0 &&
          ...
          (prevExpType !== EXPRESSION_TYPE_TOKEN || prevType === TOKEN_TYPE_USER || prevType === TOKEN_TYPE_PLACESYMBOL) &&
  ...
  previewSuggestion(data) {
    ...
    if (tokenData && get(data, 'data.value') && data.section !== SECTION_TYPE_HISTORY) {
      ...
      if (start > -1) {
        ...
        this.$el.find('.ed_tk_' + this.cursorWordData.cursorToken.data.id).addClass('ed_tk_preview').attr('preview-text', previewText);
      } else {
  ...
  getStem() {
    ...
    for (let i = cursorExp.children.length; i > 0;i--) {
      ...
      if (end <= cursorToken.data.start) {
        if (item.expressionType === EXPRESSION_TYPE_TOKEN && (item.data.type === TOKEN_TYPE_USER || item.data.type === TOKEN_TYPE_SPACE)) {
