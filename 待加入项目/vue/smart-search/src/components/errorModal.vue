...
<template>
  ...
  >
    <div class="error-label">
      {{ $tc('smartSearch.errorsTipNum', errorModalData.count, { num: errorModalData.count }) }}
    </div>
...
import { DOM_EVENT_MOUSE_MOVE, DOM_EVENT_MOUSE_LEAVE } from '../constants'
...
export default {
  ...
  methods: {
    bindEvents() {
      let self = this
      $(SELECTOR_ERROR_CONTENT)
        .off(DOM_EVENT_MOUSE_MOVE)
        .on(DOM_EVENT_MOUSE_MOVE, '.error', function (e) {
      ...
      $(SELECTOR_ERROR_CONTENT)
        ...
        .on(DOM_EVENT_MOUSE_LEAVE, '.error', function () {
          self.errorTipData = {}
        })
    },
