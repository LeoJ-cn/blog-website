...
export default {
  ...
  methods: {
    baseMixin__bindKeywordDownEvents() {
      this.$editor
        .off(DOM_EVENT_KEYDOWN).on(DOM_EVENT_KEYDOWN, (e) => {
          ...
          if ((keyCode === DOM_VK_TAB || keyCode === DOM_VK_RETURN) && this.dropData && this.dropData.data) {
            this.putSelectedOptionIntoEditor(TYPE_SELECT_OPTION_RIGHT, e);
            e.preventDefault();
            return;
          }

          if (this.dropData && this.suggestionList.length > 0 && (keyCode === DOM_VK_UP || keyCode === DOM_VK_DOWN)) {
            e.preventDefault();
        ...
        }).off(DOM_EVENT_UP).on(DOM_EVENT_UP, (e)=>{
          const { keyCode } = e;
          if (keyCode === DOM_VK_RIGHT || keyCode === DOM_VK_LEFT) {
            this.getSuggestion();
          }
        })
        .off(DOM_EVENT_FOCUS).on(DOM_EVENT_FOCUS, () => {
          this.baseMixin__updateEditorStatus(DOM_EVENT_FOCUS);
        })
        .off(DOM_EVENT_BLUR).on(DOM_EVENT_BLUR, () => {
