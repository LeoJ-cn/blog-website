...
export default {
  ...
  watch: {
    inputModeMixin__query() {
      this.inputModeMixin__query = this.inputModeMixin__query.replace(/</g, '&lt;')
        .replace(/>/g, '&gt;');
  ...
  methods: {
    ...
    inputMixn__scrollToFocus(range) {
      ...
      if (start > 0) {
        let pStr = this.inputModeMixin__query.substring(0, start) + '<span class="editor-container__textarea-target">0</span>' + this.inputModeMixin__query.substring(start);
