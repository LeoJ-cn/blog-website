...
export default {
  ...
  methods: {
    divModeMixin__bindInputEvents() {
      ...
      // 中文输入时需要使用该事件
      this.$editor
        .off(DOM_EVENT_COMPOSITION_START)
        .on(DOM_EVENT_COMPOSITION_START, () => {
          this.divModeMixin__isInputing = true
        })
        .off(DOM_EVENT_COMPOSITION_END)
        .on(DOM_EVENT_COMPOSITION_END, () => {
          this.divModeMixin__isInputing = false
        })
        .off(DOM_EVENT_INPUT)
        .on(DOM_EVENT_INPUT, this.divModeMixin__preUpdateQuery)

      this.$editor
        .parent('.ai-common-smart-search')
        .off(DOM_EVENT_MOUSE_LEAVE)
        .on(
          DOM_EVENT_MOUSE_LEAVE,
          '.editor-container--show-error .error',
          this.divModeMixin__clearErrorTip,
        )
        .off(DOM_EVENT_MOUSE_MOVE)
        .on(DOM_EVENT_MOUSE_MOVE, '.editor-container--show-error .error', function (e) {
          vm.divModeMixin__showErrorTip(this, e)
        })
        .off(DOM_EVENT_CLICK)
        .on(DOM_EVENT_CLICK, '.editor-container--show-error .field.error', function () {
