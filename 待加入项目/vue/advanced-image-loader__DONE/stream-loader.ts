...
function drawImageToCanvas(
  ...
  const _canvasWrapperGetter =
    typeof canvasWrapperGetter === 'function' ? canvasWrapperGetter : () => canvasWrapperGetter
  ...
  const currentLoaderArrive$ = scheduledThumbLoader$.pipe(
    filter(({ uuid }) => uuid === options.uuid),
  )
