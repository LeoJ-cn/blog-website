  ...
  methods: {
    clearEvent() {
      ...
      if (this.$container) {
        ...
        this.$container
          .off('mouseover', anchorSelector)
          .off('mouseout', anchorSelector)
          .off('click', anchorSelector)
      }
    },
    ...
    bindPreviewDialog(containerSelector, anchorElement, isSimpleMode) {
      ...
      $container
        .on('mouseover', anchorSelector, function () {
          clearTimeout(timer)
          let obj = $(this)
      ...
      $dialog
        .on('click', '.image-preview-left-arrow', function (e) {
          e.stopPropagation()
          if (imageIndex < 1) {
            return
          }
