<template>
    <a class="nail-box" target="_blank">
        ...
        >
            <div :key="`canvas-wrapper_${JSON.stringify(model)}`" ref="canvasElementWrapper" class="nail-box__image-wrapper"></div>
        </div>
        <slot />
    </a>
</template>
...
export default {
    ...
    watch: {
        image(value, old) {
            ...
            if (value.type === old.type && value.type !== IMAGE_TYPE_OFFICIAL && value.model.path === old.model.path) {
                return;
            }
    ...
    methods: {
        drawImage() {
            ...
            if (this.type === IMAGE_TYPE_OFFICIAL) {
                ...
                this.$_abort = drawImageToCanvas(path, canvasWrapperGetter, origin, box, this.imgSize, (err) => {
                    this.status = err ? this.FAILURE : this.DONE;
                    if (err) {
                        if (process.env.NODE_ENV !== 'production') {
                            console.warn(`Loading Image with error: <${err.name}> ${err.message}:`);
...
.nail-box {
    ...
    &__image-wrapper, &__image-canvas {
